...
#         print('cdp=',cdp1.shape)
#         x = x*cdp1+cdp2
#         print('cdimout=',x.shape)
#         return x
class CDIM(nn.Module):
    def __init__(self, dim, spa_dim=8):  # [修改] 增加 spa_dim 参数，默认 8
        super().__init__()
        self.dim = dim
        ...
        self.spa_proj = nn.Sequential(
            nn.Conv2d(spa_dim, dim * 2, kernel_size=3, padding=1),
            nn.GELU()
        )
        nn.init.zeros_(self.deg_proj[0].weight)
        nn.init.zeros_(self.deg_proj[0].bias)
        nn.init.zeros_(self.spa_proj[0].weight)
        nn.init.zeros_(self.spa_proj[0].bias)

    def forward(self, x, deg, spa, x_size):
        """
        Inputs:
            x: [B, H*W, C]
            deg: [B, 256]
        ...
        deg_params = self.deg_proj(deg).view(B, 1, C * 2)  # [B, 1, 2C]
        gamma_deg, beta_deg = deg_params.chunk(2, dim=-1)

        x = x * (1 + gamma_deg) + beta_deg

        spa_aligned = F.interpolate(spa, size=(H, W), mode='bilinear', align_corners=False)

        spa_params = self.spa_proj(spa_aligned)

        # [B, 2C, H, W] -> [B, H*W, 2C]
        spa_params = rearrange(spa_params, "b c h w -> b (h w) c")

        gamma_spa, beta_spa = spa_params.chunk(2, dim=-1)

        x = x * (1 + gamma_spa) + beta_spa

        return x


class CDRB(nn.Module):
    # ... (保持 __init__ 参数不变，但在实例化 CDIM 时传入 spa_dim) ...
    def __init__(self, dim, num_heads, reso=64, split_size=[2, 4], shift_size=[1, 2], expansion_factor=4.,
        ...
        self.cdim1 = CDIM(dim, spa_dim=spa_dim)
        self.cdim2 = CDIM(dim, spa_dim=spa_dim)
    ...
    def forward(self, x, deg, spa, x_size):
        ...
        """
        H, W = x_size
        # [修改] 这里的调用必须传入 x_size，因为 CDIM 需要它来做 interpolate
        x = x + self.drop_path(self.attn(self.cdim1(self.norm1(x), deg, spa, x_size), H, W))
        x = x + self.drop_path(self.ffn(self.cdim2(self.norm2(x), deg, spa, x_size), H, W))
...
class ResidualGroup(nn.Module):
    ...
    def forward(self, x, x_size, deg, spa):
        """
        Input: x: (B, H*W, C), x_size: (H, W)
        Output: x: (B, H*W, C)
        """
...
class UpsampleOneStep(nn.Sequential):
    """OneStep module (the difference with Upsample is that it always only has 1conv + 1pixelshuffle)
...
class FeatureFusion(nn.Module):
    def __init__(self, embed_dim):
        ...
        self.attn = nn.Sequential(
            nn.Conv2d(embed_dim * 2, embed_dim // 8, 1),
            nn.LeakyReLU(0.1),
            nn.Conv2d(embed_dim // 8, 2, 1),
            nn.Softmax(dim=1)
        )

    def forward(self, lr_feat, spa_feat):
        # lr_feat和spa_feat都是(B, C, H, W)
        weights = self.attn(torch.cat([lr_feat, spa_feat], dim=1))
        return lr_feat * weights[:, 0:1] + spa_feat * weights[:, 1:2]
...
class CDFormer_SR(nn.Module):
    ...
    def __init__(self,
        ...
        # self.cdp_change = DegspaFusion()
        self.content_proj = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
    ...
    def forward_features(self, x, deg, spa):
        _, _, H, W = x.shape
        x_size = [H, W]
        x = rearrange(x, "b c h w -> b (h w) c")
        # print(x.shape)
        ...
        for layer in self.layers:
            x = layer(x, x_size, deg, spa)
        x = self.norm2(x)
        x = rearrange(x, "b (h w) c -> b c h w", h=H, w=W)

        return x

    def forward(self, x, deg, spa):
        """
        Input: x: (B, C, H, W)
        """
        # print('11',x.shape,deg.shape,spa.shape)
        x = self.conv_first(x)
        # x_spa = self.content_proj(spa)

        # x_fused = self.fusion(x, spa)
        # print('111',x.shape)
        # cdp = self.cdp_change(deg, spa)
        # x = self.conv_after_body(self.forward_features(x_fused, deg, spa)) + x
        x = self.conv_after_body(self.forward_features(x, deg, spa)) + x

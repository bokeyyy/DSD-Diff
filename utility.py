...
class checkpoint():
    ...
    # 获取数据并移除通道维度 [1, H, W] -> [H, W]
    #   normalized = save_list[0][0].data.mul(255 / self.args.rgb_range)
    #  ndarr = normalized.byte().cpu().numpy().squeeze(0)
    # print(ndarr.shape)
    # 用 OpenCV 保存（自动处理单通道）
    # cv2.imwrite('{}.png'.format(filename), ndarr)
    #  def save_results(self, dir_in, filename, save_list, scale):
    #      filename = '{}/{}_x{}_'.format(dir_in, filename, scale)

    #      normalized = save_list[0][0].data.mul(255 / self.args.rgb_range)
    #      ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()
    # misc.imsave('{}{}.png'.format(filename, 'SR'), ndarr)
    #      imageio.imsave('{}{}.png'.format(filename, 'SR'), ndarr)
    def save_result(self, dir_in, filename, img, scale):
        filename = '{}/{}_x{}_'.format(dir_in, filename, scale)

        normalized = img[0][0].data.mul(255 / self.args.rgb_range)
        ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()
        # misc.imsave('{}{}.png'.format(filename, 'SR'), ndarr)
